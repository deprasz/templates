...
@Scope("desktop")
public class CreateAccount extends SelectorComposer<Component>{
	...
	private void doLoadData(){
		...
		Comboitem itemAdmin = new Comboitem();
		itemAdmin.setLabel(Role.ADMINISTRATOR.name());
		itemAdmin.setValue(Role.ADMINISTRATOR.getValue());
		
		Comboitem itemSupervisor = new Comboitem();
		itemSupervisor.setLabel(Role.SUPERVISOR.name());
		itemSupervisor.setValue(Role.SUPERVISOR.getValue());
		
		cmbRoleId.appendChild(itemAdmin);
		cmbRoleId.appendChild(itemSupervisor);
	}
		...
		try {
			...
			if (user == null) {
				if (mainService.findUserByEmail((String) ComponentUtil.getValue(txtEmail)) != null) {
					...
					user.setLanguage("0");
					user.setRoleId((String) ComponentUtil.getValue(cmbRoleId));
					mainService.saveUser(user);
					userActivityService.addActivity(authService.userDetails().getUserId(), 
							Activity.NON_TRANSACTION, request.getRemoteAddr(), "Create Account " + ComponentUtil.getValue(txtUserId));
	...
	private boolean doValidation(){
		...
		}else if (ComponentUtil.getValue(cmbRoleId) == null || ComponentUtil.getValue(cmbRoleId).equals("")) {
			throw new WrongValueException(cmbRoleId, "Please select role id");
